...
def stop_game():
    ...
    global user_score, current_emotion

    if current_emotion is None:
        messagebox.showwarning("Game Error", "No emotion has been chosen yet.")
        return

    # Calculate the highest percentage of the emotion assigned to the user
    analysis = DeepFace.analyze(cap.read()[1], actions=['emotion'], enforce_detection=False)
    emotions = analysis[0]['emotion']
    max_emotion = max(emotions, key=emotions.get)
    score = emotions[max_emotion]

    # Add the score to the Leaderboard
    lboard.append((name_entry.get(), current_emotion, score))
